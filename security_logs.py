import random
from utils import random_file_path, random_user, random_ip, is_attack_wave
...
def lateral_movement_log():
    techniques = [
        "SMB remote execution", "WMI lateral movement",
        "Pass-the-Hash", "Remote Service Creation",
        "PsExec execution", "RDP lateral movement"
    ]
...
def generate():
    ...
    benign_generators = [
        ...
        ("edr_suspicious_process", lambda: edr_suspicious_process(False)),
        ("av_log",                lambda: av_log(False)),
        # av_log apare de 3x — mai multe loguri benigne AV
    ]
    ...
    if malicious:
        name, func = malicious_generators[
            _generator_index % len(malicious_generators)
        ]
    else:
        name, func = benign_generators[
            _generator_index % len(benign_generators)
        ]
